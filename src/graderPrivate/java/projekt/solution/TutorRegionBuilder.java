...
public class TutorRegionBuilder implements Region.Builder {
    ...
    public Region build() {
        ...
        TutorRegion region = new TutorRegion(distanceCalc);
        nodes.forEach((l, n) -> {
    ...
    private static class NodeBuilder {
        ...
        NodeImpl build(Region region) {
            return new TutorNode(region, name, location, Collections.unmodifiableSet(connections));
        }
    }
    ...
    private static class NeighborhoodBuilder extends NodeBuilder {
        ...
        @Override
        TutorNeighborhood build(Region region) {
            return new TutorNeighborhood(region, name, location, Collections.unmodifiableSet(connections));
        }
    }
    ...
    private static class RestaurantBuilder extends NodeBuilder {
        ...
        @Override
        TutorRestaurant build(Region region) {
            return new TutorRestaurant(region, name, location, Collections.unmodifiableSet(connections), availableFood);
        }
    }
    ...
    static final class EdgeBuilder {
        ...
        TutorEdge build(Region region, DistanceCalculator distanceCalculator) {
            double distance = distanceCalculator.calculateDistance(locationA, locationB);
            long duration = (long) Math.ceil(distance);
            return new TutorEdge(region, name, locationA, locationB, duration);
        }
