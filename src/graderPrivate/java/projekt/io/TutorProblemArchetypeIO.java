...
public class TutorProblemArchetypeIO {
        ...
        try {
            ...
            while (!Objects.equals(line = reader.readLine(), "END PROBLEM ARCHETYPE")) {
                ...
                if (line.startsWith("RE")) {
                    region = TutorRegionIO.readRegion(reader);
                } else if (line.startsWith("V")) {
                    if (region == null) {
                        throw new RuntimeException("region has to be parsed before vehicleManager");
                    }
                    vehicleManager = TutorVehicleManagerIO.readVehicleManager(reader, region);
                } else if (line.startsWith("O")) {
                    if (vehicleManager == null) {
                        throw new RuntimeException("vehicleManager has to be parsed before orderGeneratorFactory");
                    }
                    orderGeneratorFactory = TutorOrderGeneratorFactoryIO.readOrderGeneratorFactory(reader, vehicleManager);
                } else if (line.startsWith("RA")) {
                    if (vehicleManager == null) {
                        throw new RuntimeException("vehicleManager has to be parsed before raterFactoryMap");
                    }
                    raterFactoryMap = TutorRaterFactoryMapIO.readRaterFactoryMap(reader, vehicleManager);
    ...
    public static void writeProblemArchetype(BufferedWriter writer, ProblemArchetype problem) {
        try {
            ...
            writer.write("RE \n");
            TutorRegionIO.writeRegion(writer, problem.vehicleManager().getRegion());

            writer.write("V \n");
            TutorVehicleManagerIO.writeVehicleManager(writer, problem.vehicleManager());

            writer.write("O \n");
            TutorOrderGeneratorFactoryIO.writeOrderGeneratorFactory(writer, problem.orderGeneratorFactory());

            writer.write("RA \n");
            TutorRaterFactoryMapIO.writeRaterFactoryMap(writer, problem.raterFactoryMap());
